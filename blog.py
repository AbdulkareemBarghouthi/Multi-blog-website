...
class SignOut(Handler):
    def get(self):
        if (not self.request.cookies.get("user_id")):
            self.render("errorPage.html", error="You are not signed in to signout!")
        else:
...
class likeFunction(Handler):
    def get(self, post_id):
        ...
        post = db.get(key)
        if (cookieHolder == post.user.Username):
            self.render("mainpage.html", allPosts = allPosts, ack = "you can't like your own post")            
        else:
            post.Likes += 1
            post.put()
...
class dislikeFunction(Handler):
    def get(self, post_id):
        ...
        post = db.get(key)
        if (cookieHolder == post.user.Username):
            self.render("mainpage.html", allPosts = allPosts, ack = "you can't dislike your own post")
        else:
            post.Dislikes += 1
            post.put()
            self.render("mainpage.html", allPosts = allPosts)
...
app = webapp2.WSGIApplication([('/blog/main', MainPage),
                            ...
                            ('/blog/dislike/(\d+)', dislikeFunction)], debug=True)
